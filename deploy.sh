...
if [ -z $IS_GREEN  ];then # blue가 실행중인 경우
  ...
  REQUEST=$(curl http://127.0.0.1:3002) # green으로 request
  ...
  NGINX_UPSTREAM="green:3000"
  sed -i "s/proxy_pass http:\/\/.*:3000;/proxy_pass http:\/\/$NGINX_UPSTREAM;/" ./nginx/default.conf
  ...
  REQUEST=$(curl http://127.0.0.1:3001) # blue로 request
